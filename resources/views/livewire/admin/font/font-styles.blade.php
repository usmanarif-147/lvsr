    ...
    <div>
        <div class="d-flex justify-content-between">
            ...
            <h5 class="card-header">
                <a class="btn btn-primary" href="{{ url('admin/font-style/create') }}"> Create
                </a>
            </h5>
        </div>
    </div>
                ...
                <div class="table-responsive text-nowrap">
                    <table class="table admin-table">
                        ...
                        <tbody class="table-border-bottom-0">
                            @foreach ($fontStyles['fontStyles'] as $fontStyle)
                                <tr>
                                    <td> {{ $fontStyle->font_style }}</td>
                                    <td> {{ $fontStyle->name }}</td>
                                    <th>
                                        {{ $fontStyle->type == 1 ? 'Free' : 'Pro' }}
                                    </th>
                                    <td>
                                        {{ defaultDateFormat($fontStyle->created_at) }}
                                    </td>
                                    <td>
                                        <span class="badge {{ model_status($fontStyle)['background'] }} me-1">
                                            {{ model_status($fontStyle)['status'] }}
                                        </span>
                                    </td>
                                    <td class="action-td">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end" aria-labelledby="cardOpt3">
                                                <a class="btn btn-icon btn-outline-secondary" data-bs-toggle="tooltip"
                                                    ...
                                                    href="{{ url('admin/button-color/' . $fontStyle->id . '/edit') }}">
                                                    <i class="bx bx-edit-alt"></i>
                                                </a>
                                                @if ($fontStyle->status == 0)
                                                    <button class="btn btn-icon btn-outline-secondary"
                                                        ...
                                                        wire:click="activateConfirmModal({{ $fontStyle->id }})">
                                                        <i class="fa fa-eye"></i>
                                                    </button>
                                                @else
                                                    <button class="btn btn-icon btn-outline-secondary"
                                                        ...
                                                        wire:click="deactivateConfirmModal({{ $fontStyle->id }})">
            ...
            <div class="row">
                <div class="col">
                    <div class="demo-inline-spacing">
                        @if ($fontStyles['fontStyles']->count() > 0)
                            {{ $fontStyles['fontStyles']->links() }}
                        @else
